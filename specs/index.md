...
- **主な構成**: ヒーロー（AIが量産する時代の品質保証。）→ サービス → 事例 → 流れ → スタイル → なぜ今 → 自社ツール（QA系6カードのみ）→ 代表 → お問い合わせCTA（#contact）
- **固定ルール**:
...
- **手動確認観点**: ヒーローのQAバグ装飾のSP非表示／サービスカードのhover演出がSPで非表示／CTAのメールとGoogleフォームが生きているか
