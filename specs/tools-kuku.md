...
- **3つの もんだい**。画面の見出しは「もんだい」。ボタン名どおりの出しかたにする（2026-09-16に作り直し。元の「しき」「ぎゃくびき」はやめた）:
  ...
  - `junban`（じゅんばんに とく）: 「2 × 1」「2 × 2」…と **1つの段を ×1 から ×9 まで順に9問**。
    だんが「ランダム」のときは段を1つくじ引きする（81問を順に出すと1の段しか出ないため）
  - `random`（ランダムに こたえる）: 選んだ範囲を **まぜて重ならずに**出す。1つの段なら9問、「ランダム」なら10問。
    「1の段」を選んだまま押すと1の段だけになるのは仕様（範囲はだんで決める）
  - 1セットの問題数が9と10でちがうので、星は数ではなく **正解の割合** で決める（100%＝3・80%＝2・50%＝1）
