...
(function () {
  ...
  var GRADES = (function () {
    ...
    if (gs.indexOf(JUKKEN) >= 0) {
      // じゅけんとっくんも ほかの学年と同じ見た目にする（2026-09-24。以前は紺地に金文字の is-jukken）
  ...
  // メニューは段階式（2026-09-22）。といかた → 学年 →（学期のある学年だけ）がっき。
  ...
  function renderMenuStep(step, keep) {
    ...
    var btn;
    if (step === 1) {
      wrap.appendChild(group('やりたい しゅぎょうを えらんでね！', MODES, 'modeId', 'kj-choices-column kj-choices-mode', 1));
    ...
    if (step === 1) {
      ...
      back = el('button', 'kj-back', '← しゅぎょうを えらびなおす');
      back.type = 'button';
