...
- 入口ボタンは左から **LPページ**（塗りなし・グレー枠＋グレー文字 `#7a7a88`、`.nav-cta-lp`）→ `/`、**QA支援**（黒塗り `#1c1c2e`、`.nav-cta-qa`）→ `/test-tools.html`、**教育支援**（BEETLE赤塗り `--color-accent`、`.nav-cta-edu`）→ `/edu-tools.html`。事業メインページでは自分自身のボタンを非表示（common.js）
