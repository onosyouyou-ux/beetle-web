...
- **アプリのヘッダー統一（2026-07-15改定・アプリ側は未反映）**：LP側は2026-07-15にテキストリンクのグローバルメニュー化済み（「グローバルメニュー」の章参照）。**Vercelアプリ3つ（bug-checker・gakkyu-tsushin・rubi-shokunin）は旧ボタン型のまま＝これから揃える**。予定：①ナビの「トップ・QA支援・教育支援」をLP側と同じテキストリンクに変更 ②**リファレンスはヘッダーから外し、メインビジュアル（page-header）内のタイトル・注釈の下に配置**（オレンジ `#ef8a3c` 角ボタンのトーンは維持）。**ランディングがないツールはリファレンスを置かない**（本体配信の静的ツールは共通パーシャル `#site-header` をそのまま使う。例：テストコンテンツ作成ツール）
...
- 塗りボタンは廃止。唯一の例外は**アプリ画面のリファレンスボタン**（オレンジ `#ef8a3c` 角ボタン。静的アプリは common.js の `staticApps` が `.nav-cta-ref` を注入）
