...
- グローバルメニューは左から **トップ**（`/`）・**QA支援**（`/test-tools.html`）・**教育支援**（`/edu-tools.html`）・**コラム**（`/blog/`）の**素のテキストリンク4つ**（色 `#55555f`・太字・hoverでアクセント色）。**現在ページは `aria-current="page"` でアクセント色**（共通ヘッダーは common.js が付与、トップLPはHTML直書き）。**4リンクとも全画面常設・SPでも非表示や「QA」等への短縮はしない**（テキストなので収まる）
