...
- **アプリのヘッダー統一（2026-07-14改定）**：Vercelアプリ（bug-checker・gakkyu-tsushin・rubi-shokunin）のヘッダーは本体と同型の `.site-nav`＋**ボタンのみ**（テキストリンク廃止）。構成は「**トップ**（アウトライングレー→本体）＋**QA支援**（黒）＋**教育支援**（BEETLE赤）＋**リファレンス**（一番右。各ランディングへ。全アプリ共通でオレンジ `#ef8a3c`＝赤の教育支援との色被り回避）」。**ボタン3つ（トップ・QA支援・教育支援）は全画面共通**（2026-07-14決定。画面によってボタンが欠けると違和感が出るため、事業系統での出し分けはしない）。**ランディングがないツールはリファレンスボタンを置かない**（本体配信の静的ツールは共通パーシャル `#site-header` をそのまま使う。例：テストコンテンツ作成ツール）。SPはロゴ＋ボタンを1行維持・ラベル短縮（`.nav-cta-full` 非表示で「QA」「教育」）
...
- 入口ボタンは左から **トップ**（塗りなし・グレー枠＋グレー文字 `#7a7a88`、`.nav-cta-lp`）→ `/`、**QA支援**（黒塗り `#1c1c2e`、`.nav-cta-qa`）→ `/test-tools.html`、**教育支援**（BEETLE赤塗り `--color-accent`、`.nav-cta-edu`）→ `/edu-tools.html`。ボタン間隔は詰めめ（gap 11px）。**3ボタンは全画面常設**（2026-07-14決定。自分自身のページでも非表示にしない。画面によってボタンが欠けると違和感が出るため）
...
- トップLP（`index.html`）のヘッダーは2段構成（2026-07-13決定）：1段目「ロゴ＋トップ・QA支援・教育支援ボタン（色は同上。トップは2026-07-14に追加＝全画面3ボタン統一）」、2段目「サービス・事例・流れ・なぜ今・スタイル・ツール・代表」中央揃え（SPは横スクロール）。相談するCTA・ハンバーガーは廃止
...
- 共通ヘッダーのボタンは出し分けしない（2026-07-14廃止）：全画面「トップ・QA支援・教育支援」の3ボタン固定。ラベルは「教育支援」（旧「教育・こども」）
- **コラムボタン（PCのみ。2026-07-14追加）**：教育支援の右に「コラム」（アウトライングレー、トップと同型）→ `/blog/`。**SP（共通ヘッダー480px以下・トップLP768px以下）では非表示**。共通ヘッダー＝`.nav-cta-blog`、トップLP＝`.header-entry.blog`、アプリ＝`.site-nav-blog`（アプリではリファレンスの左）
