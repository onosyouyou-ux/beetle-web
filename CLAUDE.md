...
- **本体配信の静的アプリ**（えいごよんで）も同じ紙面ルール。モザイク原本は `/assets/images/bg-mosaic.png`。ヘッダーは共通パーシャル＋common.jsの `staticApps` 設定で「トップ＋教育支援＋リファレンス（オレンジ）」を注入。更新日は手動更新
...
- **アプリのヘッダー統一（2026-07-13改定）**：Vercelアプリ（bug-checker・gakkyu-tsushin・rubi-shokunin）のヘッダーは本体と同型の `.site-nav`＋**ボタンのみ**（テキストリンク廃止）。構成は「**トップ**（アウトライングレー→本体）＋**事業ボタン**（QA系＝QA支援・黒／教育系＝教育支援・BEETLE赤）＋**リファレンス**（一番右。各ランディングへ。QA系＝BEETLE赤／教育系＝オレンジ `#ef8a3c`（赤の教育支援との色被り回避））」。**ランディングがないツールはリファレンスボタンを置かない**（本体配信の静的ツールは共通パーシャル `#site-header` をそのまま使う。例：テストコンテンツ作成ツール）
