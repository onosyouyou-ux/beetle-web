...
- **盤面の大きさも変数で管理する**（2026-08-28・チケット#1）。基準は**とけい修行**で、他の修行アプリをこれに合わせる
  - `--nk-qh` … 問題エリア（左カラム）の高さ。**回答エリア（右カラム）も同じ高さに伸ばして天地を揃える**
  - `--nk-board-h` … 盤面としゅぎょう選択画面の最小高さ。画面が切り替わっても箱が動かない
  - `--nk-ask-h` … とけいだけ「といのバー＋時計ステージ」の合成なので、その差分
  - 段ごとの値（PC 312/446・高さ不足 270/392・タブレット横 252/402）。**1カラムの段（スマホ・タブレット縦・寝かせた画面）は `--nk-board-h:0` にして「1画面に収める」ルールを優先する**
  - 盤面の行は `grid-template-rows:auto var(--nk-qh) auto` ＋ `align-content:start`。**`1fr` にすると盤面の最小高さぶん問題エリアが勝手に伸び、`--nk-qh` どおりにならない**（実際に踏んだ）
  - しゅぎょう選択画面は盤面と同じ `width:min(100%,1000px)`。中身が間延びするアプリ（とけい・かんじ）は**幅ではなく `padding-inline:max(20px,calc((100% - 860px)/2))` で絞る**
  - **「← もんだいせんたくに もどる」は盤面の一番下・中央揃え**に全アプリ共通で置く
  - 回答エリアは `align-content:stretch` で問題エリアと同じ高さまでボタンを伸ばす。ただし
    **2択・時計の絵・入力欄（1行しかない並び）と、寝かせた画面の段は中身なりの高さ**にする（伸ばすと1個が巨大になる／1画面に収まらない）
  - 問題エリアの地紋は `/assets/images/ninja/backgrounds/q-shippou.png`（七宝つなぎ・白線の透過PNG・1200x680）を
    `::before` に `opacity:.09` で重ねる。**白線1枚を5アプリで共用**し、修行ごとの色は下地のグラデーションで出す。
    生成は PowerShell System.Drawing（[画像取り込みワークフロー]と同じ理由でWSLにPILなし）
  - 選択肢のボタンと「アプリ一覧／紹介ページ」は `var(--nk-accent)` で修行ごとに色を変える。数値の色は直書きしない
...
- **対象は「操作するところ」**。メニューの選択肢とスタート、プレイ盤面が1画面に入ること。
  ルール解説・対応表のような**読み物は下に続いてよい**（カタカナ・ローマ字・フォニックスのメニュー下部がこれ）。
  盤面だけ直してメニューを見落とした失敗があるので、**メニュー画面も必ず測る**（2026-08-25）
- **いま選んでいるものが分かること**。v3のスキンが `border:0` にしたせいで各アプリの `.is-on`（枠の色替え）が
  効かなくなり、初期選択がどれか分からなくなっていた。選択状態は `ninja-kids.css` 側で
  「内側2pxのアクセント枠＋薄い塗り＋✓」として一括で当てる（2026-08-25修正）
...
2. Claude Code がフォルダの最新画像を拾って仕上げる：
   - 1200×800 に中央クロップ・リサイズ（画像加工の道具は下の「画像加工に使う道具」を見る）
...
### 画像加工に使う道具（2026-09-02更新）

**Windows 側の Python に Pillow 12.3（WebP対応）が入っている。** リサイズ・フォーマット変換はこれを使う。
WSL 側には無いので、Bashツール（Git Bash）から `python` を呼べばよい。

- **PowerShell System.Drawing** は WebP を書けず、PNG の圧縮も調整できない。地紋タイルの生成のように
  「描いて作る」用途だけに使う
- **写真・イラストは WebP(quality=90) が基本**。実測で PNG の 1/5 になる
  （○×フィードバック4枚：1254px PNG 合計 5.4MB → 512px WebP 合計 285KB）
- **表示サイズの2倍を上限に縮める**。240px 表示の絵に 1254px は要らない

```python
from PIL import Image
im = Image.open(src).convert("RGBA")
im.resize((512,512), Image.LANCZOS).save(dst, "WEBP", quality=90, method=6)
```

### アプリの紹介用画像は「ヒーロー＋クリックバナー」で1セット（2026-09-02決定）

アプリ（ツール）の紹介用画像を作るときは、次の2種を**同時に**発注する。片方だけ作らない。

1. **ヒーロー画像** — アプリ内 `assets/images/{app}-hero-people.jpg`（960×640）／ランディング `assets/images/{app}-lp-hero.jpg`（1080×720）
2. **本文差し込み用のクリックバナー** — `assets/images/blog/banner-{app}.jpg`（1200×400・JPG 120KB前後）。
   コラム記事の本文中ほどに貼り、ランディングへ送る

**どちらも「実際のアプリ画面」を入れる。作り方はハイブリッド**（ChatGPTだけでもスクショだけでも作らない）：

| 工程 | 担当 | 内容 |
|---|---|---|
| 1. 枠の絵 | ChatGPT | 子ども＋**空のタブレット**まで。**画面の中は薄いグレー（#d8d8d8）の無地**・タブレットは**真正面で傾けない**・画面部分は**4:3**・反射や光沢を描かせない・**文字は一切入れさせない** |
| 2. 画面 | Claude Code | ローカルに静的サーバを立て **1024×768** でアプリを開き、**プレイ盤面**（問題と選択肢が出ている画面）を撮る |
| 3. 合成 | Claude Code | グレーの無地部分にスクショを長方形で貼り込む。**位置と大きさは1枚ずつ目視で決める**（自動検出しない）。加工は PowerShell System.Drawing |
| 4. 文字 | Claude Code | **アプリ名を余白側に合成**（人物やタブレットにかぶらない側）。太めの丸ゴシック・濃い色、正式名表記（「カタカナ修行」のように漢字の「修行」）。バナーはその下に小さく「▶ ここをクリックしてためす」 |

- ChatGPTに文字を描かせないのは、**書体と位置を複数枚で揃えるため**。ここだけは合成側で持つ
- タブレットを傾けさせない・画面に反射を描かせないのは**合成の都合**。守られていないと貼り込めず作り直しになる
- 複数アプリぶんをまとめて作るときは、**先に1枚で体裁を決めてから残りを作る**（1枚ずつ発注すると絵柄がぶれる）
- 発注カード（GitHub Issue）には**サイズとどんな絵か**を必ず書く
- **カードには絵柄の見本画像を貼る**（2026-09-02決定）。文章だけだと毎回キャラが変わる。
  `<img src="https://raw.githubusercontent.com/onosyouyou-ux/beetle-web/main/assets/images/...">` で
  既存の絵を埋め込み、**ChatGPTにもその画像を貼ってから**プロンプトを渡す。
  本番に出したくない参照用の画像は `content/card-references` ブランチに置いて raw URL で参照する
- **子ども向けアプリの人物は お母さん＋女の子 で固定**（`kanji-hero-people.jpg` / `tokei-hero-people.jpg` が基準）。
  同じ部屋・同じ服装・同じ画風で描く

バナーの貼り方（記事側）：

```html
<a href="/tools/{app}/landing.html"><img class="post-banner" src="/assets/images/blog/banner-{app}.jpg" width="1200" height="400" loading="lazy" alt="カタカナ修行を使ってみる"></a>
```

- `.post-banner` は `css/blog.css`（無ければ追加。`width:100%;height:auto;border-radius:12px;margin:24px 0;`）
- 記事末尾のCTAとは**別に**、導入で困りごとを書いたあたりに1枚置く
...
- **画面のH1は かな表記**（「かんじしゅぎょう」）。小1でも読めるようにするため。
  `title`・OGP・JSON-LD は検索向けに漢字（「かんじ修行」）とする
- **ランディングの `title` は「検索語｜説明｜○○修行」の語順**（2026-09-17改定）。検索語（「時計の読み方 練習」「九九 練習・暗記」）を
  先頭に置き、ブランド名は末尾に回す。例：`時計の読み方 練習｜小学生向け 何時何分がわかる無料アプリ｜とけい修行`。
  旧ルールの「ブランド名を先頭・検索語を後半」から改めた。検索結果では先頭の語が目に入りやすく、ブランド名はまだほとんど検索されないため
...
- グローバルメニューは左から **トップ**（`/`）・**QA支援**（`/test-tools.html`）・**教育支援**（`/edu-tools.html`）・**アプリ**（`/apps.html`）・**コラム**（`/blog/`）の**素のテキストリンク5つ**（2026-09-10に「アプリ」を追加。作ったものを端から見たい人の受け皿が無かったため）（色 `#55555f`・太字・hoverでアクセント色＋下線 `text-underline-offset:6px`。常時下線は入れない）。**現在ページは `aria-current="page"` でアクセント色＋下線**（共通ヘッダーは common.js が付与、トップLPはHTML直書き）。**5リンクとも全画面常設**。SP（480px以下）では「QA」「教育」に短縮表示する（`<span class="nav-trim">支援</span>` をCSSで非表示。そのぶん文字サイズ13pxを維持して押しやすく。2026-07-19決定、旧「短縮しない」ルールを改定）
