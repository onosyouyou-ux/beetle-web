...
- **スマホ共通フッター（2026-07-10決定）**：全ページ「浮世絵バナー縮小（全幅フィット）→ BEETLEテキストロゴ → ナビリンク → プライバシーポリシー・©」の順で全て中央揃え。2段フッターLPはSP時のみ `.footer-heroes-sp` でバナー表示。共通ヘッダーのSPナビリンクも中央揃え（ハンバーガー化はしない）
