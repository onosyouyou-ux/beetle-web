...
- ナビの入口ボタン（QA支援・教育・こども・アプリのリファレンス）は**アウトライン型の角ボタン**（border 1px・radius 2px・hoverで塗りつぶし）。**丸ピル（radius 999px）は使わない**。色はQA支援＝黒・教育＝BEETLE赤
- カード末尾のリンク表示は色付きバッジ（緑の「無料」・オレンジの「→ 使ってみる」等）**禁止**。「左2pxアクセントボーダー＋太字テキスト」で統一し、文言は**「詳しく見る →」**・文字色は**アクセント色**（トップ・test-tools・edu-tools・コラムカード共通）
...
- 共通ヘッダー（`partials/header.html`）右端に角ボタン2つ（アウトライン型・radius 2px）：**QA支援**（黒 `#1c1c2e`）→ `/test-tools.html`、**教育・こども**（BEETLE赤 `--color-accent`、`.nav-cta-edu`）→ `/edu-tools.html`
...
- **`test-tools.html`＝QA事業メインページ**：01 BEETLEができること → 02 QA支援サービス4商品（デグレ確認・テスト設計・リリース前診断・月額伴走）→ 03 テスト検証用ツール → 04 リファレンス。教育系ツールカードは置かない
