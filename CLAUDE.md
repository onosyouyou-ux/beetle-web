...
1. **ヒーロー画像** — アプリ内 `assets/images/{app}-hero-people.jpg`（960×640）／ランディング `assets/images/{app}-lp-hero.jpg`（1080×720）。
   **画像の中にアプリ名を入れる**。人物にかぶらない余白側（構図が人物左寄りなら右、中央やや右なら左）に、太めの丸ゴシック・濃い色で。
   表記は正式名（「カタカナ修行」のように漢字の「修行」）。**それ以外の文字・英字・ロゴは入れない**
2. **本文差し込み用のクリックバナー** — `assets/images/blog/banner-{app}.jpg`（**1200×400**・JPG 120KB前後）。
   コラム記事の本文中ほどに貼り、ランディングへ送る。**左3割に絵、右7割は文字用の余白**。
   アプリ名を大きく、その下に「▶ ここをクリックしてためす」を小さく（バナーが押せると分かる見た目にする）
...
- 文字入れは ChatGPT 側でやってよい（日本語文字が正確）。発注カード（GitHub Issue）には**サイズとどんな絵か**を必ず書く
