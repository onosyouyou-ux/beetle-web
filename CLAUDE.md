...
## 事業入口の2ピル（2026-07-13決定）

QA事業と教育事業の二本立てに合わせ、入口を分ける：

- 共通ヘッダー（`partials/header.html`）右端にアクセント色ピル2つ：**QA支援**（`--color-accent`）→ `/test-tools.html`、**教育・こども**（オレンジ `#ef8a3c`、`.nav-cta-edu`）→ `/edu-tools.html`
- **`test-tools.html` はQA特化**：テスト検証用ツール＋リファレンス＋QA支援サービス4商品（デグレ確認・テスト設計・リリース前診断・月額伴走）への直リンク。教育系ツールカードは置かない
- **`edu-tools.html` が教育系の入口**：おうちで使える（えいごよんで・ルビメーカー）＋先生向け（学級通信メーカー）＋教育分野でできること（アプリ制作・コンテンツ制作・PTA向けページ制作）。CSSは `test-tools.css` を流用し `/css/edu-tools.css` で温かいオレンジに上書き
