...
- ナビの入口ボタン（QA支援・教育・こども・アプリのリファレンス）は**アウトライン型の角ボタン**（border 1px・radius 2px・hoverで塗りつぶし）。**丸ピル（radius 999px）は使わない**
- カード末尾のリンク表示は色付きバッジ（緑の「無料」・オレンジの「→ 使ってみる」等）**禁止**。トップページ `.tool-badge` と同じ「左2pxアクセントボーダー＋太字テキスト」で統一し、文言は**「ひらく →」**
...
- 共通ヘッダー（`partials/header.html`）右端に角ボタン2つ（アウトライン型・radius 2px）：**QA支援**（`--color-accent`）→ `/test-tools.html`、**教育・こども**（オレンジ `#ef8a3c`、`.nav-cta-edu`）→ `/edu-tools.html`
