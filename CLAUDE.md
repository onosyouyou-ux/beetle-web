...
- **フッターの系統（2026-07-15改定。PC・スマホ同一レイアウト、全て中央揃え）**：
  - **アプリ共通フッター（全アプリ統一。2026-07-15）**：bug-checker・gakkyu-tsushin・rubi-shokunin・えいごよんで の全アプリ画面で同一。bg `#1a1a1a`・リンク色 `#888`。1段目「BEETLEテキストロゴ＋ナビ（トップ・QA支援・教育支援・コラム・お問い合わせ→`/#contact`）」・2段目「プライバシーポリシー・会社名」。クラスは `.site-footer-app` ＞ `.sfa-row`（ロゴ＋ナビ）＋ `.sfa-copy`。**テスト検証用ツール（bug-checker）だけ浮世絵ヒーローズバナー（`.footer-heroes`）をフッターの上に表示**（バナーは紙面幅1180px）
  - **本体共通パーシャル `#site-footer`**：浮世絵ヒーローズバナー → ナビリンク＋プライバシーポリシー・会社名を**1行中央揃え**（狭い画面では折り返し）
...
- **アプリのヘッダー統一（2026-07-15改定・全アプリ反映済み）**：全アプリ（bug-checker・gakkyu-tsushin・rubi-shokunin・えいごよんで）のナビはLP側と同じ**テキストリンクのグローバルメニュー**（トップ・QA支援・教育支援・コラム。SPでも4リンクフル表示）。**リファレンスはヘッダーに置かず、メインビジュアル内のタイトル・注釈の直下**にオレンジ `#ef8a3c` 角ボタン（`.ph-ref`）で置く。**ランディングがないツールはリファレンスを置かない**（本体配信の静的ツールは共通パーシャル `#site-header` をそのまま使う。例：テストコンテンツ作成ツール）
...
- グローバルメニューは左から **トップ**（`/`）・**QA支援**（`/test-tools.html`）・**教育支援**（`/edu-tools.html`）・**コラム**（`/blog/`）の**素のテキストリンク4つ**（色 `#55555f`・太字・hoverでアクセント色＋下線 `text-underline-offset:6px`。常時下線は入れない）。**現在ページは `aria-current="page"` でアクセント色＋下線**（共通ヘッダーは common.js が付与、トップLPはHTML直書き）。**4リンクとも全画面常設・SPでも非表示や「QA」等への短縮はしない**（テキストなので収まる）
